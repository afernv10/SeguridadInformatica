...
public class Practica {
	...
	public static  void imprimir() {
		for(Letra l : letras) {
			...
			System.out.println(l.toString()+"\tFrecuencia["+l.getFrecuencia()+"]\tProbabilidad["+String.format("%.3f",l.getProbabilidad())+"]");
		}
	}
...
class ComparadorNombre implements Comparator<Letra> {
    ...
    public int compare(Letra l1, Letra l2) {
    	...
    	if (l1.getProbabilidad() < l2.getProbabilidad()) return 1;
        if (l1.getProbabilidad() > l2.getProbabilidad()) return -1;
        return 0;
    }
